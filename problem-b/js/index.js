...
function compoundInterest(initBalance, annualInterestRate, years) {
  return Number((initBalance * Math.pow(Math.E, (annualInterestRate * years))).toFixed(2));
}
...
console.log(getLetterFrequencies("Mmmmm, to Yogoslavia, I must go"));
...
const cardCount = 52;
const suitNum = 13;
for (let i = 0; i < cardCount / suitNum; i++) {
...
function containsQueenOfHearts(deck) {
  for (let i = 0; i < deck.length; i++) {
    if (deck[i].rank === 12 && deck[i].suit === "hearts") {
      return true;
    }
  }
  return false;
